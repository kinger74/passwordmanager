...
class password_things(): #יאני כל הדברים של הההצפנה וזה של הססמא אין לי מושג איך לקרוא לזה
    def encrypt_password(password: str) -> str:
        fernet = Fernet(encryption_key.load_key())
    ...
    try:
        ...
        if command == "REGISTER":
            if len(parts) == 3:
                ...
                response = user_functions.register_user(username, password)
            ...
            if len(parts) == 3:
                username, password = parts[1], parts[2]
                response = user_functions.login_user(username, password)
                client_socket.send(response.encode('utf-8'))
            else:
            ...
            if len(parts) == 5:
                ...
                response = user_functions.save_password(username, service, service_username, password)
                client_socket.send(response.encode('utf-8'))
            else:
            ...
            if len(parts) == 2:
                username = parts[1]
                response = user_functions.get_all_passwords(username)
                client_socket.send(response.encode('utf-8'))
            else:
            ...
            if len(parts) == 3:
                ...
                response = user_functions.delete_password(username, service)
                client_socket.send(response.encode('utf-8'))
            else:
...
class user_functions(): #כל הפונקציות של המשתמשים   
    ...
    def register_user(username, password):
        ...
        hashed_password = password_things.hash_password(password)
    ...
    def login_user(username, password):
        ...
        if result and password_things.check_password(result[0], password):
    ...
    def save_password(username, service, service_username, password):
        ...
        if user:
            ...
            encrypted_password = password_things.encrypt_password(password)
    ...
    def get_password(username, service, service_username): #retrieve a password for a user
        ...
        if result:
            try:
                decrypted_password = password_things.decrypt_password(result[0])
    ...
    def get_all_passwords(username): #retrieve all passwords for a user
        ...
        try:
            ...
            for service, service_username, encrypted_pass in passwords:
                try:
                    decrypted = password_things.decrypt_password(encrypted_pass)
...
if __name__ == "__main__":
    ...
    create_db()
    encryption_key.generate_key()  # Uncomment this line once to generate a new key
