...
class loggedin_functions():
    ...
    def save_password():
        ...
        loggedin_functions.get_all_passwords()
    ...
    def get_all_passwords():
        ...
        try:
            ...
            loggedin_functions.populate_delete_options(response)
        except Exception as e:
            password_text.insert(tk.END, "no passwords found")
    ...
    def auto_fill_credentials():
        ...
        try:
            ...
            messagebox.showerror("Error", f"Failed to auto-fill- service might not be supported: {str(e)}")
            driver.quit()
...
register_button = tk.Button(login_frame, text="Register", command=login_commands.register_user)
register_button.grid(row=3, column=1, padx=5, pady=5)

login_button = tk.Button(login_frame, text="Login", command=login_commands.login_user)
...
save_button = tk.Button(password_management_frame, text="Save Password", command=loggedin_functions.save_password)
save_button.grid(row=3, column=0, padx=2, pady=5)

get_all_button = tk.Button(password_management_frame, text="Get All Passwords", command=loggedin_functions.get_all_passwords)
...
delete_button = tk.Button(password_management_frame, text="Delete Password", command=loggedin_functions.delete_password)
delete_button.grid(row=6, column=0, columnspan=3, padx=5, pady=5)

autofill_button = tk.Button(password_management_frame, text="Autofill Password", command=loggedin_functions.auto_fill_credentials)
autofill_button.grid(row=6, column=0, columnspan=1, padx=5, pady=5)

logout_button = tk.Button(password_management_frame, text="Logout", command=loggedin_functions.logout)
